...
public class OrderController {
    ...
    @GetMapping("/models")
    public View getModels(@RequestParam Long makeId, Model model) {
        final var models = orderService.getModelsByMakeId(makeId);
        final var id = "modelId";
        final var name = "modelId";
        model.addAttribute("id", id);
        model.addAttribute("name", name);
        model.addAttribute("models", models);
        return FragmentsRendering
                .fragment("fragments/model-select :: modelSelect")
                .build();
    }

    @PostMapping("/step1")
    public View processStep1(@Valid @ModelAttribute("productSelectionForm") ProductSelectionForm productSelectionForm,
        ...
        if (result.hasErrors()) {
            model.addAttribute("makes", orderService.getAllMakes());
            model.addAttribute("currentStep", 1);
            return new ThymeleafView("step1");
        }

        Order order = sessionService.getOrder(session);
        orderService.updateProductSelection(order, productSelectionForm);
        sessionService.setOrder(session, order);

        return new HtmxLocationRedirectView("/order/step2");
    }
    ...
    @PostMapping("/step2")
    public View processStep2(@Valid @ModelAttribute("customizationForm") CustomizationForm customizationForm,
                               BindingResult result,
                               Model model,
                               HttpSession session) {
        if (result.hasErrors()) {
            model.addAttribute("currentStep", 2);
            return new ThymeleafView("step2");
        }
        ...
        return new HtmxLocationRedirectView("/order/step3");
    }
    ...
    @PostMapping("/step3")
    public View processStep3(@Valid @ModelAttribute("deliveryInformationForm") DeliveryInformationForm deliveryInformationForm,
                               BindingResult result,
                               Model model,
                               HttpSession session) {
        if (result.hasErrors()) {
            model.addAttribute("currentStep", 3);
            return new ThymeleafView( "step3");
        }
        ...
        return new HtmxLocationRedirectView("/order/step4");
    }
    ...
    @PostMapping("/step4")
    public View processStep4(@Valid @ModelAttribute("paymentInformationForm") PaymentInformationForm paymentInformationForm,
                               BindingResult result,
                               Model model,
                               HttpSession session) {
        if (result.hasErrors()) {
            model.addAttribute("currentStep", 4);
            return new ThymeleafView("step4");
        }
        ...
        return new HtmxLocationRedirectView( "/order/step5");
    }
    ...
    @PostMapping("/step5")
    public View processStep5(HttpSession session) {
        Order order = sessionService.getOrder(session);
        Order savedOrder = orderService.confirmOrder(order);
        sessionService.removeOrder(session);
        return new HtmxLocationRedirectView("/order/success?orderId=" + savedOrder.getId());
    }
